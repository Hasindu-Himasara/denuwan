...
MAIL_GRABBER = 'https://www.penpalsnow.com/_api/showemail.php?e='
...
async def multipages(target=10, found=0, session=None, gender='female', age_group='19-22', country='UK', num=0,
                     bot=None, update=None):
    if session is None:
    ...
    for i in range(2, len(paras)):
        ...
        address = str(paras[i].find('address').find('span', class_='ppadvalue').text).strip().replace("\n",
                                                                                                      ',').replace(
            '       ', ' ')
        email = requests.get(MAIL_GRABBER + ids[index]).text
...
help_string += "\n📎<b>STRUCTURE ->  </b><code>penpals_t_g_a_c</code>"
...
async def penpals_main(bot, update):
    ...
    if len(words) == 5:
        ...
        await write_log(f'{update.chat.id} - penpalsnow t - {target} g - {gender} a - {age_group} c - {country}')
        await multipages(target, found=0, gender=gender, age_group=AGES[age_group], country=COUNTRIES[country.lower()],
                         num=0, bot=bot, update=update)
